...
const ETSY_URL_PATTERN = /^https?:\/\/(www\.)?etsy\.com\//;

/**
 * Import reviews from Etsy.
 * Supports pasted text (user copies reviews from shop page) and URL scraping.
 */
export async function importFromEtsy(
  input: string,
  type: "url" | "csv"
  ...
  if (ETSY_URL_PATTERN.test(input)) {
...
async function extractFromURL(url: string): Promise<ImportedReview[]> {
  ...
  try {
    ...
    throw new Error(
      "Could not fetch Etsy page. Try copying the reviews and pasting them instead."
    );
  }
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are a review extraction assistant. Extract customer reviews from the given Etsy shop/product page HTML.
...
Extract the reviewer name, star rating (1-5), review text, and date if available.`,
      ...
      {
        role: "user",
        content: `Extract all customer reviews from this Etsy page HTML:\n\n${trimmed}`,
      },
    ],
  });
  ...
  try {
    ...
    return (parsed.reviews || []).map((r: Record<string, unknown>) => ({
      reviewer_name: String(r.reviewer_name || "Etsy Buyer"),
...
The text is copied from an Etsy shop page.
Return JSON: { "reviews": [{ "reviewer_name": "...", "rating": 5.0, "review_text": "...", "review_date": "..." }] }
If no reviews are found, return { "reviews": [] }.`,
      },
      {
        role: "user",
        content: `Extract all reviews from this Etsy page text:\n\n${trimmed}`,
      },
    ],
  });
